...
if [ -e "$DEPLOYMENT_TARGET/package.json" ]; then
  ...
  eval /opt/nodejs/10.13.0/bin/npm install --production
  echo "test message 9997 $DEPLOYMENT_TARGET"
  eval /opt/nodejs/10.13.0/bin/npm install --only=dev
...
if [ -e "$DEPLOYMENT_TARGET/.angular-cli.json" ]; then
  cd "$DEPLOYMENT_TARGET"
  pwd
  eval /opt/nodejs/10.13.0/bin/node /home/site/wwwroot/node_modules/.bin/ng build
